...
void pre_scan(void)
{
    ...
    do {
        ...
        if (tok != VOID && tok != CHAR && tok != INT && tok != FLOAT && tok != DOUBLE)
            show_error(NOT_VAR_OR_FUNC_OUTSIDE);
    ...
    switch (into_type) {
        case DT_CHAR:
            switch (data_to_convert->type) {
                ...
                default:
                    show_error(INVALID_TYPE_CONVERSION);
            }
...
void exec_while(void)
{
    ...
    while (cond.value.i) {
        ...
        if (jump_flag == JF_BREAK || jump_flag == JF_RETURN)
            break; // if a continue, break or return command has been found, then this loop stops
...
void exec_for(void)
{
    ...
    for (; cond.value.i;) {
        interp_block();
        if (jump_flag == JF_CONTINUE)
            jump_flag = JF_NULL; // resets the jump flag, if a continue statement has been found, so that the loop continues to run normally
        else if (jump_flag == JF_BREAK || jump_flag == JF_RETURN)
            break; // if either a break or a return command has been found, this stops the loop
...
// if no jump flag has been found, then finds the end of the block, since after every loop, the program pointer is reset to the beginning of the block
    if (jump_flag == JF_NULL) find_end_of_block();
    else if (jump_flag == JF_BREAK)
        jump_flag = JF_NULL; // resets the flag, in case a break command was found, but not if a return command was found
}
...
void eval_relational(_DATA *v)
{
    ...
    eval_terms(v);
    while (tok == EQUAL || tok == NOT_EQUAL || tok == LESS_THAN || tok == GREATER_THAN || tok == LESS_THAN_OR_EQUAL ||
...
void eval_atom(_DATA *v)
{
    ...
    if (tok == MINUS) {
        ...
        if (tok == OPENING_BRACKET) { // matrix operations
            ...
            char i;
            char data_size; // matrix data size
            _DATA index;
            int dims;

            if (!is_matrix(
                    temp_name)) { // if the variable is not a matrix, then it must be a pointer. gives the variable value and returns, so that eval_matrix can work on it.
...
//check local variables first, from the previous stack index to the current one
    for (i = local_var_tos_history[user_func_call_index - 1]; i < local_var_tos; i++)
        if (!strcmp(local_variables[i].var_name, var_name)) {
            ...
            convert_data(v,
                         local_variables[i].data.type); // converts the expression contained in v to the type defined by the variable, before assigning
    ...
    for (i = 0; i < global_var_tos; i++)
        if (!strcmp(global_variables[i].var_name, var_name)) {
            ...
            convert_data(v,
                         global_variables[i].data.type); // converts the expression contained in v to the type defined by the variable, before assigning
...
// *********************************************************************************************
        ...
        if (tok == OPENING_BRACKET) {
            ...
            if ((global_variables[global_var_tos].data.value.p = malloc(matrix_mem)) == NULL)
                show_error(MEMORY_ALLOCATION_FAILURE);
        }
...
char *add_string(void)
{
    ...
    if ((string_table[string_table_tos] = (char *) malloc(strlen(string_constant) + 1)) == NULL)
        show_error(MEMORY_ALLOCATION_FAILURE);
...
int find_local_var(char *var_name)
{
    ...
    for (i = local_var_tos_history[user_func_call_index - 1];
         (i < local_var_tos) && (*local_variables[i].var_name); i++)
